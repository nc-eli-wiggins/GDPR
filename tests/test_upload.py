...
def test_upload_file_to_s3_valid_input(s3_upload_setup):
    ...
    with patch("os.path.isfile", return_value=True), patch(
        "src.utils.upload.s3.upload_file"
    ) as mock_s3_upload:
...
def test_upload_file_to_s3_generates_unique_timestamped_filename():
    ...
    with patch("os.path.isfile", return_value=True), patch(
        "src.utils.upload.datetime"
    ) as mock_datetime, patch("src.utils.upload.s3.upload_file"):
    ...
    [
        ...
        (
            ...
            "test-key",
            {
                "Body": MagicMock(
                    read=MagicMock(
                        return_value=json.dumps({"outputs": {}}).encode("utf-8")
                    )
                )
            },
            (None, None, None),
...
def test_get_bucket_names_from_tf_state(
    mock_get_object, bucket_name, object_key, mock_response, expected, test_id
):
