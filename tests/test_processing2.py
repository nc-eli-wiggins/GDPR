...
from src.utils.processing2 import get_bucket_names_from_tf_state, obfuscate_pii, get_keys_from_bucket, empty_bucket, handler, logger, s3
...
def mock_aws_s3():
    with mock_aws():
        ...
        s3.put_object(
            Bucket=bucket_name,
            Key=object_key,
            Body=json.dumps(state_file_content)
        )

        yield bucket_name, object_key, (mock_input_bucket_name, mock_processed_bucket_name, mock_invocation_bucket_name)

def test_get_bucket_names_from_tf_state(mock_aws_s3):
    bucket_name, object_key, expected_buckets = mock_aws_s3

    input_bucket_name, processed_bucket_name, invocation_bucket_name = get_bucket_names_from_tf_state(bucket_name, object_key)
...
@pytest.mark.parametrize(
    ...
    [
        ("test-bucket", "test.csv", ["email"], "name,email\nJohn,john@example.com", "name,email\nJohn,***\n"),
        
        ("test-bucket", "test.csv", ["email", "phone"], "name,email,phone\nJohn,john@example.com,1234567890", "name,email,phone\nJohn,***,***\n"),
        
        ("test-bucket", "test.csv", [], "name,email\nJohn,john@example.com", "name,email\nJohn,john@example.com\n"),
        
        ("test-bucket", "test.csv", ["nonexistent"], "name,email\nJohn,john@example.com", "name,email\nJohn,john@example.com\n"),
    ],
    ids=[
        "single_pii_field",
        "multiple_pii_fields",
...
def test_obfuscate_pii_happy_and_edge_cases(mock_logger, mock_s3, bucket_name, s3_file_path, pii_fields, csv_content, expected_output):
...
@pytest.mark.parametrize(
    ...
    [
        ("test-bucket", "test.csv", ["email"], Exception("S3 error"), "Failed to process file: S3 error"),
        
        ("test-bucket", "test.csv", ["email"], pd.errors.EmptyDataError("No columns to parse from file"), "Failed to process file: No columns to parse from file"),
    ],
    ids=[
        "s3_exception",
        "invalid_csv_format"
...
@patch('src.utils.processing2.s3')
@patch('src.utils.processing2.logger')
def test_obfuscate_pii_error_cases(mock_logger, mock_s3, bucket_name, s3_file_path, pii_fields, exception, log_message):
...
@pytest.mark.parametrize(
    ...
    [
        ("test-bucket", {'Contents': [{'Key': 'data.json'}]}, 'data.json', "single_json_key"),
        
        ("test-bucket", {'Contents': [{'Key': 'data.txt'}, {'Key': 'data.json'}, {'Key': 'image.png'}]}, 'data.json', "multiple_keys_one_json"),
        
        ("test-bucket", {}, None, "no_keys"),
        
        ("test-bucket", {'Contents': [{'Key': 'first.json'}, {'Key': 'second.json'}]}, 'first.json', "multiple_json_keys"),
        
        ("test-bucket", {'Contents': [{'Key': 'data.txt'}, {'Key': 'image.png'}]}, None, "no_json_key"),
...
@patch('src.utils.processing2.logger')
def test_get_keys_from_bucket(mock_logger, mock_list_objects, bucket_name, mock_response, expected_key, test_id):
...
@pytest.mark.parametrize("bucket_name, list_objects_response, expected_log", [
    ("test-bucket", {'Contents': [{'Key': 'file1'}, {'Key': 'file2'}]}, "All objects deleted from bucket: test-bucket"),
    ("empty-bucket", {}, "No objects found in bucket: empty-bucket"),
], ids=["bucket_with_objects", "empty_bucket"])
def test_empty_bucket_happy_path(bucket_name, list_objects_response, expected_log, caplog):
    with patch('src.utils.processing2.s3') as mock_s3:
        ...
        if 'Contents' in list_objects_response:
            assert mock_s3.delete_object.call_count == len(list_objects_response['Contents'])
        else:
            assert mock_s3.delete_object.call_count == 0
        assert expected_log in caplog.text

@pytest.mark.parametrize("bucket_name, exception, expected_log", [
    ("nonexistent-bucket", ClientError({"Error": {"Code": "NoSuchBucket"}}, "ListObjectsV2"), "Failed to delete objects from bucket: An error occurred (NoSuchBucket)"),
    ("access-denied-bucket", ClientError({"Error": {"Code": "AccessDenied"}}, "ListObjectsV2"), "Failed to delete objects from bucket: An error occurred (AccessDenied)"),
...
import pytest
from unittest import mock
from src.utils.processing2 import handler  
...
def test_handler_success(mock_empty_bucket, mock_obfuscate_pii, mock_put_object, mock_s3_get_object, mock_get_keys, mock_get_bucket_names):
    event = {}
    context = {}

    mock_get_bucket_names.return_value = ('input-bucket', 'processed-bucket', 'invocation-bucket')

    mock_get_keys.return_value = 'data.json'

    mock_s3_get_object.return_value = {
        'Body': mock.Mock(read=mock.Mock(return_value=json.dumps({
...
@mock.patch('src.utils.processing2.get_keys_from_bucket')
@mock.patch('src.utils.processing2.s3.get_object')
def test_handler_500_no_json_file_found(mock_s3_get_object, mock_get_keys, mock_get_bucket_names):
    event = {}
    context = {}
    ...
    assert json.loads(response['body']) == 'Error retrieving JSON file from invocation bucket.'


@mock.patch('src.utils.processing2.get_bucket_names_from_tf_state')
@mock.patch('src.utils.processing2.get_keys_from_bucket')
@mock.patch('src.utils.processing2.s3.get_object')
def test_handler_500_error_reading_json_file(mock_s3_get_object, mock_get_keys, mock_get_bucket_names):
    event = {}
    context = {}
...
@mock.patch('src.utils.processing2.s3.get_object')
@mock.patch('src.utils.processing2.obfuscate_pii')
def test_handler_500_error_processing_json_content(mock_obfuscate_pii, mock_s3_get_object, mock_get_keys, mock_get_bucket_names):
    event = {}
    context = {}
